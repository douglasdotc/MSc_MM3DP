...
%                 q_new               = node_SE2(this.TD_Extend(q_rand, q_nearest, delta_l_step, delta_l, dist_method));
                ...
                % if this.CollisionCheck(q_new)
                    ...
                    if cost_sol ~= Inf
                        this.cost_store{1}          = [this.cost_store{1}, cost_sol];
                        this.sigma_store{1, end+1}  = sigma_sol;
                        [~, idx]                    = sort(this.cost_store{1}, 2, 'descend');
                        this.cost_store{1}          = this.cost_store{1}(idx);
                        this.sigma_store(1, :)      = this.sigma_store(1,idx);
                    end
                    ...
                    if IsForward % It is processing forward tree now
                        if q_new.pose(5) > s_max_forward % Use an array to store!!!!!!
                    ...
                    if ~isempty(this.cost_store{1}) 
                        if size(this.cost_store{1}, 2) >= 10 && mean(abs(this.cost_store{1}(end - 6:end - 1) - this.cost_store{1}(end - 5:end))) <= Bi_cost_threshold
                            this.cost_best  = this.cost_store{1}(end);
                            this.sigma_best = this.sigma_store{1,end};
                            Reached_Goal(1) = true;
                        end
                    end
        ...
        function [sigma_sol, cost_sol, edges] = Connect(this, q_new, q_connect, tree_backward, delta_l_step, delta_l, eps_neigh, dist_method)
            ...
            for jdx = 1:length(X_near_no_collision)
                if C_near_no_collision(jdx) < this.cost_best
                    ...
                    fprintf("Path merged between progress %.4f, %.4f with cost %.4f\n", q_new.pose(5), X_near_no_collision(jdx).pose(5), cost_sol);
                    return
                end
            end
            edges     = this.edges;
