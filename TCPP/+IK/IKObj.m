classdef IKObj < handle
    ...
    methods
        ...
        function setup_default_cart_msg(self)
            ...
            start_state = robotics.ros.msggen.moveit_msgs.RobotState;
            start_state.JointState.Name = self.joint_names;
            start_state.JointState.Position = zeros(1, length(self.joint_names));
            self.cart_request_msg.StartState = start_state; %TODO
        ...
        function answer = get_cartesian(self, task, frame, starting_conf)
            %  task is (t) local
            self.cart_request_msg.StartState.JointState.Position = starting_conf;
            poseArray = ros_helpers.mat2ros_pose_array(task, frame);
            self.cart_request_msg.Waypoints = poseArray.Poses;
            answer = self.ik_client.call(self.ik_request_msg);
        ...
        function add_constraints(self, constraint)
            ...
            if isempty(self.ik_request_msg.IkRequest.Constraints.Name)
                self.ik_request_msg.IkRequest.Constraints.Name = "Constraints";
            end
            ...
            switch class(constraint)
                ...
                case 'ros.msggen.moveit_msgs.OrientationConstraint'
                    self.ik_request_msg.IkRequest.Constraints.OrientationConstraints = [self.ik_request_msg.IkRequest.Constraints.OrientationConstraints constraint];
                otherwise
